...
/** 
 * The DrugIDUpdater program gets drug ID for a list of drugs from our drugs
...
public class DrugIDUpdater {
	...
	public static void main(String[] args) {
		...
		try {
		    ...
		    output.add("drug_name\tdisease_name\tdrugID_from_lexicon\tdisease_cui");
			while((line = br.readLine()) != null) {
				if(line.startsWith("Drug_name")) continue;
				
				String[] arrLine = line.split("\t");
				String drugID = drugAndID.get(arrLine[0].toLowerCase());
                if(drugID==null) {
                    if(!drugIDList.contains(arrLine[2])) drugID = arrLine[2];
                }
				output.add(arrLine[0] + "\t" + arrLine[1] + "\t" + drugID + "\t" + arrLine[3]);
